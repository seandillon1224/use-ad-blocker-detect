...
const getStorageFunctions = (type: StorageType) => {
  ...
  switch (type) {
    case 'cookies': {
      ...
      storageSetter = () => Cookies.set(AD_BLOCKER_COOKIE, new Date().getTime().toString());
      return [storageStatus, storageSetter]
    }
    case 'local': {
      storageStatus = window.localStorage.getItem(AD_BLOCKER_COOKIE);
      storageSetter = () => window.localStorage.setItem(AD_BLOCKER_COOKIE, new Date().getTime().toString());
      return [storageStatus, storageSetter]
    }
    case 'session': {
      storageStatus = window.sessionStorage.getItem(AD_BLOCKER_COOKIE);
      storageSetter = () => window.sessionStorage.setItem(AD_BLOCKER_COOKIE, new Date().getTime().toString());
      return [storageStatus, storageSetter]
    }
    default: {
